...
/* Event Bindings - hashchange, submit, and click */
	...
	$.mobile._registerInternalEvents = function(){
		...
		/* Davinci에서 pageBox를 사용할 경우에 min-height가 강제로 설정되는 것을 막기위해 제거함. (ysjung)
	...
	_create: function() {
		...
		if ( type !== "button" && type !== "reset" && name ) {
				$el.bind( "vclick", function() {
					// Add hidden input if it doesn?셳 already exist.
...
$.davinci = {
	 ...
	 * @param {String} transition 전환 효과 
	 * @param {Object} data	전달 할 데이터
	 */
	...
	_preventTouch : function(flag) {
		if(flag == true) {
			var obj = $('<div></div>').css({height:'100%',width:'100%',position:'fixed',left:0,top:0,'z-index':1001,opacity:0});
			obj.css('cursor','wait');
			this.preventEl = obj.prependTo('body');
		}
		else {
			this.preventEl.remove();
		}
	},
...
$.widget( "mobile.dvcBase", $.mobile.widget, {
	...
	refresh: function(){
		...
		o.visible = $el.attr("data-visible") || true;
		o.visible = self._stringToBoolean(o.visible); 
		o.enable = $el.attr("data-enable") || true;
		o.enable = self._stringToBoolean(o.enable);
		o.customStyle = $el.attr("data-custom-style") || false;
		o.customStyle = self._stringToBoolean(o.customStyle);
	...
	_stringToBoolean: function(string){
		switch(string){
			...
			case "false":case "FALSE": case "False": case null: return false;
	...
	_addDOMAttrModifiedEventListener: function(){
		...
		var elemToCheck = $el[0];
	    if (elemToCheck.addEventListener) { // all browsers except IE before version 9
	        elemToCheck.addEventListener ('DOMAttrModified', this._OnAttrModified, false);    // Firefox, Opera, IE
	    }
	    if (elemToCheck.attachEvent) {  // Internet Explorer and Opera
	        elemToCheck.attachEvent ('onpropertychange', this._OnAttrModified);   // Internet Explorer
	    }

	},
	_OnAttrModified: function(event) {
	    var message = "";
	    if ('attrChange' in event) {    // Firefox, Opera, Internet Explorer from version 9
	        ...
	        case MutationEvent.MODIFICATION:
	            message += "The value of the " + event.attrName + " attribute has been changed from " + event.prevValue + " to " + event.newValue + ".";
				{
					var target = event.target;
					...
					if(id){
						inst = $.davinci.getInstance($target);
					}
					else if(subid){
						$parent = $target.parent().closest("div[id]");
						inst = $.davinci.getInstance($parent, subid);
					}
					inst.refresh();
				}
	            break;
	        case MutationEvent.ADDITION:
	            message += "The " + event.attrName + " attribute has been added to the element " + "with the value of " + event.newValue + ".";
	            break;
	        case MutationEvent.REMOVAL:
	            message += "The " + event.attrName + " attribute has been removed from the element." + "The value was " + event.prevValue + " previously.";
	            break;
	        };
	    }
...
$.widget( "mobile.dvcButton", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		o.text = $el.attr("data-text") || "";
		o.buttonType = $el.attr("data-button-type") || "default";
		o.upImage = $el.attr("data-up-image") || "";
		o.downImage = $el.attr("data-down-image") || "";
		o.upImageChecked = $el.attr("data-up-image-checked") || "";
		o.downImageChecked = $el.attr("data-down-image-checked") || "";
		o.borderWidths = $el.attr("data-border-widths") || "1 1 1 1";
		o.upIcon = $el.attr("data-up-icon") || "";
		o.downIcon = $el.attr("data-down-icon") || "";
		o.upIconChecked = $el.attr("data-up-icon-checked") || "";
		o.downIconChecked = $el.attr("data-down-icon-checked") || "";
		o.iconPos = $el.attr("data-icon-pos") || "left";
		o.iconMargin = parseInt($el.attr("data-icon-margin"), 10) || 0;
		o.textMargin = parseInt($el.attr("data-text-margin"), 10) || 0;
		o.checked = $el.attr("data-checked") || false;
		o.checked = self._stringToBoolean(o.checked);
		o.groupName = $el.attr("data-grout-name") || "";
		
		$el.empty();
		$el.removeClass("dvc-button-back dvc-button-cancel dvc-button-ok dvc-button-checkbox dvc-button-radio dvc-button-icon-top dvc-button-icon-right dvc-button-icon-bottom dvc-button-icon-left dvc-button-custom");
			...
			if(state == "up" && checked == false && o.upIcon) {
				...
				if(o.upIcon || o.downIcon) {
					...
					if($el.hasClass("dvc-button-custom")) {
						self.$icon[0].style.content = "none";
					}	
				}
			}
		}
...
$.widget( "mobile.dvcImageButton", $.mobile.dvcBase, {
	...
	refresh: function(){
		...
		o.src = $el.attr("data-src") || "";
		o.text = $el.attr("data-text") || "";
		o.imageType = $el.attr("data-image-type") || "";
		o.upImage = $el.attr("data-up-image") || "";
		o.downImage = $el.attr("data-down-image") || "";
...
$.widget( "mobile.dvcCollapseview", $.mobile.dvcBase, {
	...
	_create: function(){
		...
		var $newEl = $('<div  waper_focus="false" waper_dontsave="true" class="dvc-collapseview-bottom">Collapseview</div>');
		$el.append($newEl);
	...
	expand: function(e, noAnimation) {
		var $el = this.element, self = this, o = this.options;
		if ( arguments.length ) {
			o.expand = e;
			if(e) {
				$el.removeClass("dvc-collapseview-collapse").addClass("dvc-collapseview-expand");
			}
			else {
				$el.removeClass("dvc-collapseview-expand").addClass("dvc-collapseview-collapse");
			}
		}
		else {
			return o.expand;
	...
	refresh: function() {
		...
		o.expand = $el.attr("data-expand") || true;
		o.expand = self._stringToBoolean(o.expand);

		var $children = $el.children();
		var len = $children.length;
		
		this.expand(self.expand(), true);

/*
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-collapseview-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Collapseview </td></tr></table>');
			$el.append($newEl);
			this.expand(true, true);
			$el.addClass("dvc-collapseview-design-height");
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-collapseview-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
			$el.removeClass("dvc-collapseview-design-height");
			this.expand(self.expand(), true);
		}
		else{
			this.expand(self.expand(), true);
...
$.widget( "mobile.dvcHtmlview", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		o.pageid = $el.attr("data-pageid") || "";
		
		if(len == 0){
			var elHeight = $el.height();
			var $newEl = $('<div waper_focus="false" waper_dontsave="true" class="dvc-htmlview-default-design" style="background-color:rgba(50,50,50,0.3); font-weight:bold; color:white; display:-webkit-box; -webkit-box-pack:center; -webkit-box-align:center; min-height:inherit;"> Htmlview </div>');
			$el.append($newEl);
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-htmlview-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcImage", $.mobile.dvcBase , {
	...
	src: function(url) {
		...
		if ( arguments.length ) {
			...
			if(url){
				var $defaultDesign = $el.find('.dvc-image-default-design');
				...
				var $defaultDesign = $el.find('.dvc-image-default-design');
				if($defaultDesign.length == 0) {
					var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-image-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Image </td></tr></table>');
					$el.append($newEl);
				}
			}

		} else {
			return this.element.css("background-image");
		}
	},
	...
	imageType : function(type){
		var $el = this.element;
		if(type == "tile"){
			$el.addClass("dvc-image-tile").removeClass("dvc-image-stretch").removeClass("dvc-image-zoom").removeClass("dvc-image-center");
		}
		else if(type == "stretch"){
			$el.removeClass("dvc-image-tile").addClass("dvc-image-stretch").removeClass("dvc-image-zoom").removeClass("dvc-image-center");
		}
		else if(type == "zoom"){
			$el.removeClass("dvc-image-tile").removeClass("dvc-image-stretch").addClass("dvc-image-zoom").removeClass("dvc-image-center");
		}
		else if(type == "center"){
			$el.removeClass("dvc-image-tile").removeClass("dvc-image-stretch").removeClass("dvc-image-zoom").addClass("dvc-image-center");
		}
		else { //if(image_type == "none"){
			$el.removeClass("dvc-image-tile").removeClass("dvc-image-stretch").removeClass("dvc-image-zoom").removeClass("dvc-image-center");
		}
	},
	refresh: function(){
		var $el = this.element, o = this.options;
...
$.widget( "mobile.dvcLabel", $.mobile.dvcBase, {
	...
	_create: function(){
		...
		$el.addClass( "dvc-label dvc-label-up");
		
		self._findScrollview();
		
		var $divText = $("<div cLass='dvc-label-text'></div>");
		$el.append($divText);
	...
	text: function(t) {
		...
		if ( arguments.length ) {
			var $divText = $el.find(".dvc-label-text");
			if(t === ""){
				var $oldText = $el.find('.dvc-label-text');
				if($oldText.length > 0) {
					$oldText.remove();
				}
				var $newEl = $('<div waper_focus="false" waper_dontsave="true" class="dvc-label-text dvc-label-single" style="text-shadow:0 0;text-align: left; color: rgba(0, 0, 0, 0.6); font-style: italic;" >Label</div>');
				$el.append($newEl);
			}
			else{
	...
	refresh: function() {
		...
		o.text = $el.attr("data-text") || "";
		o.textAlign = $el.attr("data-text-align") || "left";
		o.multiline = $el.attr("data-multiline") || false;
		o.multiline = self._stringToBoolean(o.multiline);
		o.enable = $el.attr("data-enable") || false;
		o.enable = self._stringToBoolean(o.enable);
...
$.widget( "mobile.dvcHeader", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		var $content = $siblings.filter("div[data-role='dvcContent']");
...
$.widget( "mobile.dvcContent", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		var $el = this.element;
		var $siblings = $el.siblings();
		var $header = $siblings.filter("div[data-role='dvcHeader']");
		var $footer = $siblings.filter("div[data-role='dvcFooter']");
...
$.widget( "mobile.dvcLayoutview", $.mobile.dvcBase, {
	...
	_buttonType: function() {
		...
		$el.find(".dvc-layoutview-link-icon").remove();
		$el.find(".dvc-layoutview-checkbox-icon").remove();
		
		$el.removeClass("dvc-layoutview-none dvc-layoutview-link dvc-layoutview-up dvc-layoutview-button dvc-layoutview-checkbox dvc-layoutview-radio dvc-layoutview-checkboxradio dvc-layoutview-radio-checked dvc-layoutview-checkbox-icon-hidden");
		...
		if(o.buttonType == "none") {
			...
			var $link = $("<div waper_focus='false' waper_dontsave='true' class='dvc-layoutview-link-icon'></div>");
			$link.appendTo($el);
		} else if(o.buttonType == "button") {
			...
			var $checkbox = $("<div waper_focus='false' waper_dontsave='true' class='dvc-layoutview-checkbox-icon'></div>");
			$checkbox.clone().appendTo($el);
			
			self.$checkbox = $el.find(".dvc-layoutview-checkbox-icon");
		...
		} else if(o.buttonType == "checkboxradio") {
	...
	refresh: function() {
		...
		o.buttonType = $el.attr("data-button-type") || "none";
		o.checked = $el.attr("data-checked") || false;
		o.checked = self._stringToBoolean(o.checked);
		o.groupName = $el.attr("data-group-name") || "";

		var $children = $el.children();
		var len = $children.length;
		
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-layoutview-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Layoutview </td></tr></table>');
			$el.append($newEl);
		}
		else{
			var $defaultEl = $children.filter(".dvc-layoutview-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcListview", $.mobile.dvcBase, {
	...
	_buttonType: function() {
		...
		$el.find(".dvc-listview-link-icon").remove();
		$el.find(".dvc-listview-checkbox-icon").remove();

		$el.removeClass("dvc-listview-none dvc-listview-link dvc-listview-up dvc-listview-button dvc-listview-checkbox dvc-listview-radio dvc-listview-checkboxradio dvc-listview-radio-checked dvc-listview-checkbox-icon-hidden dvc-listview-radio-down");
		
		if(o.buttonType == "none") {
			$el.addClass("dvc-listview-none");
		} else if(o.buttonType == "link") {
			$el.addClass("dvc-listview-link dvc-listview-up");
			var $link = $("<div waper_focus='false' waper_dontsave='true' class='dvc-listview-link-icon'></div>");
			$link.appendTo($el);
		} else if(o.buttonType == "button") {
			$el.addClass("dvc-listview-button dvc-listview-up");
		} else if(o.buttonType == "checkbox") {
			$el.addClass("dvc-listview-checkbox dvc-listview-up");

			var $checkbox = $("<div waper_focus='false' waper_dontsave='true' class='dvc-listview-checkbox-icon'></div>");
			$checkbox.clone().appendTo($el);
			
			self.$checkbox = $el.find(".dvc-listview-checkbox-icon");
		} else if(o.buttonType == "radio") {
			$el.addClass("dvc-listview-radio dvc-listview-up");

			$(this).addClass("dvc-listview-radio-down");
		} else if(o.buttonType == "checkboxradio") {
			$el.addClass("dvc-listview-checkboxradio dvc-listview-up");
	...
	itemData: function(lineIndex, d) {
	},
	refresh: function(){
		var $el = this.element, o = this.options;
		
		o.buttonType = $el.attr("data-button-type") || "button";
		this._buttonType();
		$.mobile.dvcBase.prototype.refresh.call(this);
	}
...
(function($, undefined ) {
$.widget( "mobile.dvcPicker", $.mobile.dvcBase, {
	options: {
		fps:               30,    // Frames per second in msecs.
		scrollDuration:    2000,  // Duration of the scrolling animation in msecs.
		overshootDuration: 100,   // Duration of the overshoot animation in msecs.
	...
	_create: function() {
		...
		self.eventType = $.support.touch ? "touch" : "mouse";
		self.startEventName = "ev_scrollstart";
		self.stopEventName = "ev_scrollstop";

		$("<div class='dvc-picker-slots-wrapper'><div class='dvc-picker-frame'></div><div class='dvc-picker-slots'><ul></ul></div></div>").appendTo($el);
		
		this._$clip = $(this.element).children().addClass("scrollview-clip");
		var $child = this._$clip.find(".dvc-picker-slots");
		if ($child.length > 1) {
			$child = this._$clip.wrapInner("<div></div>").children();
		}
		this._$view = $child.addClass("scrollview-view");
		...
		this._vTracker = new MomentumTracker(this.options);
	...
	_startMScroll: function(speedY)
	{
		...
		this.element.trigger(this.startEventName);
	...
	_stopMScroll: function()
	{
		if (this._timerID)
		{
			this.element.trigger(this.stopEventName);
	...
	_handleMomentumScroll: function()
	{
		...
		this._setScrollPosition(y);

		if (keepGoing)
			this._timerID = setTimeout(this._timerCB, this._timerInterval);	
		else {
			// 여기가 drag시 마지막 멈추는 시점임.
			// 44px은 한 라인의 height이고, 이 값을 기준으로 라인에 맞게 pixel을 보정한다.
			this._pixelCorrection(y);
		}
	},

	_setScrollPosition: function(y)
	{
		this._sy = y;
		var $v = this._$view;
		setElementTransform($v, y + "px");
	},
	...
	scrollTo: function(y, duration)
	{
		this._stopMScroll();
		if (!duration)
			return this._setScrollPosition(y);
		...
		var tfunc = function(){
			...
			{
				self._timerID = 0;
				self._setScrollPosition(y);
			}
			else
			{
				var ec = efunc(elapsed/duration, elapsed, 0, 1, duration);
				self._setScrollPosition(sy + (dy * ec));
	...
	{
		var svh = [];
		this._$clip.parents(".scrollview-clip").each(function(){
	...
	_getAncestorByDirection: function()
	{
	...
	_handleDragStart: function(e, ey)
	{
	...
	_propagateDragMove: function(sv, e, ey)
	{
		this._disableTracking();
		sv._handleDragStart(e,ey);
		sv._didDrag = this._didDrag;
	},

	_handleDragMove: function(e, ey)
	{
		...
		if (this._vTracker)
		{
			...
			{
				var sv = this._getAncestorByDirection();
				if (sv)
				{
					this._setScrollPosition(newY > 0 ? 0 : this._maxY);
					this._propagateDragMove(sv, e, ey);
					return false;
				}
	...
	_handleDragStop: function(e)
	{
		...
		if (sy) {
			this._startMScroll(sy);
		} else {
			// drag후 scroll이 되지않고 멈추게 됨. 이때도 pixel 보정해야함.
			var y = this._sy;
	...
	_pixelCorrection: function(y, e)
	{
		...
		if(this.getScrollPosition().y == y) {
			...
			if(e != undefined) {
				...
				y = (index*o.itemHeight);
				this.scrollTo(y, o.snapbackDuration);
				
				if(index != this._oldIndex) {
					var id = this.element[0].id;
					if(!id) {
						id = this.element.attr("subid");
					}
					this.element.trigger("ev_change", [id, index]);
			...
			this.scrollTo(y, o.overshootDuration);
	...
	_addBehaviors: function()
	{
		...
		if (this.eventType === "mouse")
		{
			...
			this._dragStartCB = function(e){
				return self._handleDragStart(e, e.clientY);
			};

			this._dragMoveEvt = "vmousemove";
			this._dragMoveCB = function(e){
				return self._handleDragMove(e, e.clientY); 
			};
		...
		else // "touch"
		{
			...
			this._dragStartCB = function(e)
			{
				...
				return self._handleDragStart(e, t.pageY);
			};
			...
			{
				var t = e.originalEvent.targetTouches[0];
				return self._handleDragMove(e, t.pageY);
			};
...
function setElementTransform($ele, y)
{
	var v = "translate3d(0px," + y + ", 0px)";
	$ele.css({
		"-moz-transform": v,
		"-webkit-transform": v,
		"transform": v
...
$.widget( "mobile.dvcProgress", $.mobile.dvcBase, {
	...
	refresh: function(){
		...
		o.value = $el.attr("data-value") || 0;
		o.min = $el.attr("data-min") || 0;
		o.max = $el.attr("data-max") || 100;
	...
	$.widget("mobile.dvcSlider",$.mobile.dvcBase,{
		...
		_changehandle:function(event){
			...
			if(typeof(event)=="object"){
				...
				if(position > elBgWidth) {
					position = elBgWidth;					
				}

				var round= Math.round(position/average);	//선택한 위치의 구간값을 round시킴
				var section= round+min; 
			}else{
				if(!event){
					event = o.value;
				}
				var section= event;			//사용자가 지정하는 handle의 section값
				var round = section-min;	//handle의 section값이 들어오므로 min 값을 빼준다.
		...
		refresh:function(){
			...
			o.value = parseInt($el.attr("data-value"),10) || 0;
			o.min = parseInt($el.attr("data-min"),10) || 0;
			o.max = parseInt($el.attr("data-max"),10) || 100;
			...
			if(o.value > o.max || o.value<o.min){
				o.value=o.min;
			}
			self.value(o.value);
			$.mobile.dvcBase.prototype.refresh.call(this);
...
$.widget( "mobile.dvcSwitch", $.mobile.dvcBase , {
	...
	_change: function() {
		...
		if(this.enable() == false && self._getAncestorEnable() == true) {
			return;
		}
	...
	refresh: function(){
		...
		o.checkedLabel = $el.attr("data-checked-label") || "ON";
		o.uncheckedLabel = $el.attr("data-unchecked-label") || "OFF";
		...
		if (o.resizeHandle)    { this._optionallyResize('handle'); }
		if (o.resizeContainer) { this._optionallyResize('container'); }
...
(function($, undefined ) {
	...
	*/
	...
	refresh: function() {
		...
		this.removeAll();
		this._addDOMAttrModifiedEventListener();
		...
		rootnode.push({
			...
			"text" : "root",
			"selected" : "false",			
			"isexpand" : true,				// childnode가 펼쳐져 있는 상태
			"childnodes" : childnode		// childnode
		});
						
		this.drawTreeview(rootnode);
...
(function($, undefined ) {
$.widget( "mobile.dvcCarousel", $.mobile.dvcBase, {
	options: {
		pageHandler : ".dvc-carousel-page",
		maxShiftPage : 1,
		draglaunch: 0.5,
		friction : 0.325,
		sensitivity : 40,
	...
	refresh: function() {
		...
		o.pageHandler = $el.attr("data-page-handler") || ".dvc-carousel-page";
		o.maxShiftPage = $el.attr("data-max-shift-page") ||  1;
		o.draglaunch = $el.attr("data-draglaunch") ||  0.5;
		o.friction = $el.attr("data-friction") ||  0.325;
		o.sensitivity = $el.attr("data-sensitivity") ||  40;
		o.extrashift = $el.attr("data-extrashift") ||  800;
		o.touchduring = $el.attr("data-touchduring") ||  800;
		o.easing = $el.attr("data-easing") ||  "easeOutQuad";
		o.bounce = $el.attr("data-bounce") || false;
		o.bounce = self._stringToBoolean(o.bounce);
		o.infinite = $el.attr("data-infinite") ||  false;
		o.infinite = self._stringToBoolean(o.infinite);
		o.pageGap = $el.attr("data-page-gap") ||  0;
		
		var $children = $el.children();
		var len = $children.length;
		
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-carousel-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Carousel </td></tr></table>');
			$el.append($newEl);
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-carousel-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcCarouselIndicator", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		o.gap = $el.attr("data-gap") || 10;
		o.gap = parseInt(o.gap, 10);
...
jQuery.widget( "mobile.dvcScrollview", $.mobile.dvcBase, {
	options: {
		...
		delayedClickSelector: ".bbit-tree-node>div *, textarea, input, .dvc-listview-item, .dvc-layoutview, .dvc-label, .dvc-button, .dvc-imagebutton, .dvc-switch, .dvc-radiobutton, .dvc-checkbox, .dvc-slider",
	...
	{ 
		...
		$el.addClass( "dvc-scrollview ui-scrollview-clip" );
	...
	refresh: function(){
		...
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-scrollview-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Scrollview </td></tr></table>');
			$el.append($newEl);
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-scrollview-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcPagebox", $.mobile.dvcBase, {
	...
	activeSubpageClass: "dvc-subpageview-activepage",
	isPageTransitioning: false,
	...
	changePage: function( subpageid, options ) {
	},
	...
	refresh: function() {
		...
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-pagebox-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Pagebox </td></tr></table>');
			$el.append($newEl);
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-pagebox-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcTextarea", $.mobile.dvcBase, {
	...
	refresh: function() {
		...
		if(len == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-textarea-default-design" style="text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Textarea </td></tr></table>');
			$el.append($newEl);
		}
		else if(len == 2){
			var $defaultEl = $children.filter(".dvc-textarea-default-design");
			if( $defaultEl.length){
				$defaultEl.remove();
			}
		}
...
$.widget( "mobile.dvcTextfield", $.mobile.dvcBase , {
	...
	refresh: function(){
		$el = this.element, o = this.options;

		o.text = $el.attr("data-text") || "";
		o.inputType = $el.attr("data-input-type") || "";
		o.placeholder = $el.attr("data-placeholder") || "";
		...
		var $input = $("<input waper_focus='false' waper_dontsave='true' data-role='none' style='position:absolute; left:0; right:0' type='" + o.inputType +"'></input>");
		this._o = $input[0];
		$input.css({
...
(function($,undefined){
$.widget("mobile.dvcDivision",$.mobile.dvcBase,{
	options:{
	...
	refresh: function() {
		...
		//$el.empty();
		var dLen = $el.find(".dvc-division-default-design").length;
		if(dLen == 0){
			var $newEl = $('<table waper_focus="false" waper_dontsave="true" class="dvc-division-default-design" style="position:absolute;top:0;text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Division </td></tr></table>');
			$el.append($newEl);
		}
	}
...
$.widget("mobile.dvcVideo",$.mobile.dvcBase,{
	options:{
		initSelector: ":jqmData(role='dvcVidio')"
	},
	...
	refresh: function() {
		...
		$el.empty();
		var $newEl = $('<table waper_focus="false" waper_dontsave="true" style="position:absolute;top:0;text-shadow:0 0;width:100%; height:100%; border-collapse:collapse; background-color:rgba(50,50,50,0.3); text-align:center; font-weight:bold; color:white;" ><tr waper_focus="false" waper_dontsave="true"><td waper_focus="false" waper_dontsave="true"> Video </td></tr></table>');
		$el.append($newEl);
	}
...
$( ".ui-page" ).live( "pageshow", function(){
	...
	$page.removeClass( "ui-body-c" );
	$page.addClass("ui-page-c");
